...
int luaV_equalobj (lua_State *L, const TValue *t1, const TValue *t2) {
  ...
  if (ttypetag(t1) != ttypetag(t2)) {  /* not the same variant? */
    if (ttype(t1) != ttype(t2) || ttype(t1) != LUA_TNUMBER) {
      /* LuaJIT compat: full userdata with __eq (e.g. FFI cdata) may compare
         equal to nil or other non-same-type values via metamethod. */
      if (L != NULL) {
        ...
        else if (ttypetag(t2) == LUA_VUSERDATA) {
