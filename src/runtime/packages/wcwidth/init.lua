-- wcwidth -- Display width of a Unicode codepoint / UTF-8 string.
--
-- Reference: Markus Kuhn's wcwidth.c (2007), updated for Unicode 15
-- East-Asian Width property data. We keep the table data tight by storing
-- only the (start, end) pairs of each width class and binary-searching.
--
-- Public surface:
--   wcwidth.width(cp_or_char)  -- 1 codepoint or 1 UTF-8 char
--                              -- returns -1 for non-printable controls,
--                              -- 0 for combining marks, 1 normal, 2 wide
--   wcwidth.swidth(s)          -- sum width across a UTF-8 string,
--                              -- stripping ANSI CSI / OSC escapes

local M = {}

-- ===== Combining (zero-width) ranges ===================================
-- Drawn from Unicode 15 + Mn / Me / Cf characters that don't advance the
-- cursor. Sorted by `lo` so we can binary-search. (lo, hi) inclusive pairs.
...
local COMBINING = {
    ...
    {0x0C81, 0x0C81}, {0x0CBC, 0x0CBC}, {0x0CCC, 0x0CCD}, {0x0CE2, 0x0CE3},
    ...
    {0x1160, 0x11FF}, {0x135D, 0x135F}, {0x1712, 0x1714}, {0x1732, 0x1734},
    ...
    {0x110BD, 0x110BD}, {0x110C2, 0x110C2}, {0x11100, 0x11102},
...
-- ===== Wide (double-cell) ranges =======================================
-- Unicode 15 East-Asian Wide + Fullwidth + emoji presentation blocks.
...
local function in_range(cp, table_)
    local lo, hi = 1, #table_
    while lo <= hi do
        local mid  = (lo + hi) >> 1
        local pair = table_[mid]
        if cp < pair[1] then
            hi = mid - 1
        elseif cp > pair[2] then
            lo = mid + 1
        else
            return true
        end
...
local function decode1(s, i)
    local b1 = s:byte(i)
    if b1 == nil then return nil, i end
    if b1 < 0x80 then return b1, i + 1 end
    if b1 < 0xC2 then return 0xFFFD, i + 1 end
    if b1 < 0xE0 then
        local b2 = s:byte(i + 1) or 0
        return ((b1 & 0x1F) << 6) | (b2 & 0x3F), i + 2
    end
    if b1 < 0xF0 then
        local b2 = s:byte(i + 1) or 0
        local b3 = s:byte(i + 2) or 0
        return ((b1 & 0x0F) << 12) | ((b2 & 0x3F) << 6) | (b3 & 0x3F), i + 3
    end
    local b2 = s:byte(i + 1) or 0
    local b3 = s:byte(i + 2) or 0
    local b4 = s:byte(i + 3) or 0
    return ((b1 & 0x07) << 18) | ((b2 & 0x3F) << 12)
         | ((b3 & 0x3F) << 6)  |  (b4 & 0x3F), i + 4
end

-- ===== Public API ======================================================

local function width_cp(cp)
    if cp == 0 then return 0 end
    if cp < 32 or (cp >= 0x7F and cp < 0xA0) then return -1 end  -- C0/C1 controls
    if in_range(cp, COMBINING) then return 0 end
    if in_range(cp, WIDE)      then return 2 end
    return 1
end

function M.width(arg)
    if type(arg) == "number" then return width_cp(arg) end
    local cp = decode1(arg, 1)
    if cp == nil then return -1 end
    return width_cp(cp)
end

-- Match ANSI CSI (ESC [ ... letter) and OSC (ESC ] ... ST/BEL) sequences so
-- we can skip them when measuring colored output.
local function skip_escape(s, i)
    -- caller guarantees s:byte(i) == 0x1B
    local b = s:byte(i + 1)
    if b == nil then return i + 1 end
    if b == 0x5B then  -- CSI: ESC [
        local j = i + 2
        local len = #s
        while j <= len do
            local c = s:byte(j)
            if c >= 0x40 and c <= 0x7E then return j + 1 end
            j = j + 1
        end
        return j
    elseif b == 0x5D then  -- OSC: ESC ] ... ST(ESC \) or BEL
        local j = i + 2
        local len = #s
        while j <= len do
            local c = s:byte(j)
            if c == 0x07 then return j + 1 end                  -- BEL
            if c == 0x1B and s:byte(j + 1) == 0x5C then return j + 2 end  -- ST
            j = j + 1
        end
        return j
    else
        -- single-char Fe sequence: ESC then one byte
        return i + 2
    end
end

-- ===== Spec-compatible aliases ========================================
-- wcwidth(cp) and wcswidth(s) match the POSIX terminology used in the
-- user spec. width() / swidth() above are kept for back-compat.

function M.wcwidth(cp)
    if type(cp) == "number" then return width_cp(cp) end
    local n = decode1(cp, 1)
    if n == nil then return -1 end
    return width_cp(n)
end

function M.is_wide(cp)
    if type(cp) ~= "number" then
        local n = decode1(cp, 1)
        if n == nil then return false end
        cp = n
    end
    return in_range(cp, WIDE)
end

-- Common emoji ranges. Not exhaustive but covers the major Unicode 15
-- emoji blocks; combine with is_wide() for layout decisions.
local EMOJI_RANGES = {
    {0x1F300, 0x1F5FF},
    {0x1F600, 0x1F64F},
    {0x1F680, 0x1F6FF},
...
function M.is_emoji(cp)
    if type(cp) ~= "number" then
        local n = decode1(cp, 1)
        if n == nil then return false end
        cp = n
    end
    return in_range(cp, EMOJI_RANGES)
end

function M.wcswidth(s)
    local total = 0
    local i = 1
    local len = #s
    while i <= len do
        ...
        if b == 0x1B then
            ...
            local cp, ni = decode1(s, i)
            if cp == nil then break end
            local w = width_cp(cp)
            -- Treat unprintable controls as zero-width when summing rather
            -- than poisoning the count with -1.
...
-- back-compat alias for the original `swidth` name
M.swidth = M.wcswidth
