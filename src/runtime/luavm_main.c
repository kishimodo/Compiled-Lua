...
static int LuaVm_Msghandler( lua_State *L ) {
...
#define LUAVM_PKG_COMPRESS_HDR     8             /* magic (4) + uint32 orig len */
#define LUAVM_PKG_COMPRESS_FORMAT  4             /* COMPRESSION_FORMAT_XPRESS_HUFF */
...
static int LuaVm_PackageLoader( lua_State *L ) {
...
static void LuaVm_RegisterPreload( lua_State *L, const char *Name,
    ...
    lua_pushcclosure( L, LuaVm_PackageLoader, 3 );
    lua_setfield( L, -2, Name );                     /* preload[Name] = loader */
}

static void LuaVm_InstallPreloads( lua_State *L ) {
    lua_getglobal( L, "package" );
    lua_getfield( L, -1, "preload" );
    LuaVm_RegisterPreload( L, "windows", g_PackageWindowsLua, &g_PackageWindowsLua_len );
    LuaVm_RegisterPreload( L, "async",   g_PackageAsyncLua,   &g_PackageAsyncLua_len );
    lua_pop( L, 2 );
}
...
static void LuaVm_RegisterRuntimeTextRegion( void ) {
...
static void LuaVm_PrintError( lua_State *L ) {
    const char *Msg = lua_tostring( L, -1 );
...
/* Run the function at top of stack with nargs args (consumed) and the
   ...
   status code. */
static int LuaVm_CallProtected( lua_State *L, int NArgs, int NResults ) {
    int Base = lua_gettop( L ) - NArgs;  /* function index */
    lua_pushcfunction( L, LuaVm_Msghandler );
...
static int LuaVm_DoString( lua_State *L, const char *Src, const char *ChunkName ) {
    if ( luaL_loadbuffer( L, Src, strlen( Src ), ChunkName ) != LUA_OK ) {
        LuaVm_PrintError( L );
        return -1;
    }
    if ( LuaVm_CallProtected( L, 0, 0 ) != LUA_OK ) {
        LuaVm_PrintError( L );
        return -1;
    }
    return 0;
...
static int LuaVm_DoFile( lua_State *L, const char *Path ) {
...
static void LuaVm_PushArgTable( lua_State *L, int Argc, char **Argv, int ProgIdx ) {
...
static int LuaVm_LoadReplLine( lua_State *L, const char *Line ) {
...
static void LuaVm_PrintReplResults( lua_State *L, int Top ) {
    int N = lua_gettop( L ) - Top;
    if ( N <= 0 ) return;
    lua_getglobal( L, "print" );
    lua_insert( L, -N - 1 );
    if ( LuaVm_CallProtected( L, N, 0 ) != LUA_OK ) {
        LuaVm_PrintError( L );
    }
}

static int LuaVm_Repl( lua_State *L ) {
    fprintf( stderr, "LuaVM REPL  -- ^Z + enter to exit\n" );
    char Line[ 4096 ] = { 0 };
    for ( ;; ) {
        fputs( "> ", stderr );
        fflush( stderr );
        ...
        if ( LuaVm_LoadReplLine( L, Line ) != 0 ) {
            LuaVm_PrintError( L );
            continue;
        }
        if ( LuaVm_CallProtected( L, 0, LUA_MULTRET ) != LUA_OK ) {
            LuaVm_PrintError( L );
            continue;
        }
        LuaVm_PrintReplResults( L, Top );
    }
    return 0;
}
...
int main( int Argc, char **Argv ) {
    ...
    if ( !Veh_Init( ) ) {
        fprintf( stderr, "luavm: Veh_Init failed (GLE=%lu)\n",
                 ( unsigned long )GetLastError( ) );
        return EXIT_FAILURE;
    }
    LuaVm_RegisterRuntimeTextRegion( );

    lua_State *L = luaL_newstate( );
    if ( L == NULL ) {
        fprintf( stderr, "luavm: out of memory\n" );
        return EXIT_FAILURE;
    }
    Ffi_SetDispatchL( L );
    luaL_openlibs( L );
    Coro_OpenLib( L );
    LuaVm_InstallPreloads( L );
    LuaVm_SetupModulePath( L );
    Ctype_Init( );
    ...
    if ( Argc <= ArgBase ) {
        ...
        if ( Argc < ArgBase + 2 ) {
            ...
            LuaVm_PushArgTable( L, Argc, Argv, ArgBase + 1 );  /* arg[0]=code, arg[1..]=args */
            Rc = LuaVm_DoString( L, Argv[ ArgBase + 1 ], "=(command line)" );
        }
    } else {
        /* run a file: arg[0] = scriptname, arg[1..] = remaining argv */
        LuaVm_PushArgTable( L, Argc, Argv, ArgBase );
        Rc = LuaVm_DoFile( L, Argv[ ArgBase ] );
    }
