...
- `local a,b=2,3 local c=a+b print(c)` shrinks 21.7%, from 141,824 to 111,104
  ...
  shrinking each call site from 20 bytes to 12. Another ~4 KB comes from imm8
...
- This host cannot resolve a 10% timing effect. A control kernel that
  physically cannot be affected by the change under test came out 0.79x to 1.05x
...
- Sound-mask plus anchor split. See changed above for the size win. The
...
- Measured CRT-dependency surface in `docs/benchmarks/no-crt-baseline.md`: of 552
  ...
  static mingw). Two findings shape any CRT-free plan. First, `libmingwex.a`
  provides none of the transcendentals, so an own libm is unavoidable and the
  oracle must be rebuilt against the same libc. Second, the Lua core already
  avoids CRT `setjmp` via `__builtin_setjmp`. Note up front: the CRT is already
  outside our binaries (they import `api-ms-win-crt-*`), so `--crt=none` would
  be a self-containment and determinism feature and a net size increase. The
...
- `-O3` escape analysis plus scalar replacement (slice 1). The first M3 memory
  ...
  `Rt_GetField`/`Rt_SetField` helper calls vanish: about 38x on an alloc-heavy
  ...
  (GC-safe slot placement plus interprocedural escape). It is default-on at
  `-O3` only, and the `clua` default is `-O2`, so default builds are unaffected,
  and it costs nothing when it does not fire.
...
- The differential plus conformance oracle now runs at every selectable `-O`
...
- Smaller exes: unenforced CET instrumentation is no longer emitted. The
  ...
  never enforced: pure dead weight. Switching the shipped runtime plus core to
  `-fcf-protection=none` strips ~283 pads from a `print` hello-world
  (298 -> 15) and ~497 from rover (514 -> 17); the residual handful live in
  MinGW's prebuilt CRT startup objects, which we link verbatim. hello drops
  139 KB -> 137 KB, rover 689 KB -> 687 KB. `-fstack-protector-strong` stays:
  it is real, near-free stack-smash protection. This was the one remaining
  non-pulling flag: the size/optimizer set was already the C/Rust release shape
  (`-Os`, `--gc-sections` with `-ffunction-sections`/`-fdata-sections`,
...
- Leaner exes: the standard library is opened selectively, and the dispatch
  ...
  - `luaL_openlibs` opened the whole stdlib unconditionally, forcing every
    ...
    reference interpreter exactly. A hello-world sheds ~41 KB of .text. Works
    on the internal linker, gcc, and `--shared-rt`.
  - The dispatch cache was a fixed 1024-entry static array, ~72 KB of .bss
    per exe, plus a hard 1024-function ceiling. It is now a heap array grown
...
The first beta. The toolchain is now self-contained (a built-in COFF-to-PE64
...
- All five remaining package XFAILs are fixed at the root; they were
  peripheral FFI and OS-API quirks:
  ...
  - `xpress` tiny inputs (XPRESS-SMALL-001): XPRESS and LZNT1 cannot represent an
    ...
    plus original size, so decompress recovers them with no marker.
...
- The built-in linker is now the default; gcc is optional. `clua build`
  links with its own COFF-to-PE64 linker (`LcPe_Link`) whenever the CRT sysroot
...
- Built-in COFF-to-PE64 linker, no external toolchain. A new
  self-contained linker (`clua/src/link/{coff_read,ar_read,pe_emit}.c`,
  `LcPe_Link`) reads the codegen object plus the CLua runtime/Lua archives plus
  a snapshot of the MinGW CRT and emits a runnable stripped console PE
  directly, replacing the gcc/ld subprocess. Implements the input set's full
  link semantics: GNU-archive symbol-index pull to a fixpoint
  (first-definition-wins; explicit objects shadow archive members so
  `aot_entry.o`'s closed-world stubs hide the Lua parser), COMDAT select-any
  dedup, weak externals, COMMON into `.bss`, all seven AMD64 relocations
  (ADDR64/ADDR32/ADDR32NB, REL32 + REL32_1..5, SECREL, SECTION), `$`-suffix
  grouped/sorted sections, per-DLL import directory plus IAT plus jmp thunks
  synthesized from dlltool long members (real export name read from
  `.idata$6`, so moldname aliases like `__set_app_type`->`_set_app_type`
  resolve), the TLS data directory from `_tls_used`, base relocations (DIR64),
  and `__ImageBase`. The mixed `libucrt.a` (import stubs for a dozen
  `api-ms-win-crt-*.dll` plus real objects like `ucrt_fprintf.o`) is handled by
  per-member classification plus a normalized head-symbol-to-DLL map. Sysroot
  snapshot via `make -f build/Makefile.luac sysroot` -> `build/bin/sysroot`
  and `dist/lib/sysroot`, discovered relocatably next to the runtime archives.
  Link time drops from ~132 ms to ~76 ms (no gcc subprocess). Validated by
  `tests/unit/test_lc_pe_emit.c` and a `--ld=internal` section in
  `tools/test-clua-cli.lua`. (Landed opt-in behind `--ld=internal`; see the
  two entries above for `--gc-sections` and the default flip that made it
  gcc-free out of the box.)
...
- Opt-in shared runtime: `clua build --shared-rt` (also on `aotc`). Links
  the program against a new `clua-rt.dll` (the full AOT runtime plus Lua core,
  ...
  `dist\lib\` now ships `clua-rt.dll` plus `libclua-rt.dll.a` plus
  `protoinit_rt.o`.
...
- Foreign packages (Go-style): `rover install` and `rover add` accept
  ...
  external tools rover already uses (curl plus the tar.exe shipped with
...
- New suite `tools/test-pkgmgr-foreign.lua`: GitHub URL parsing plus name
  derivation (via the `ROVER_PKG_TEST` hook), registry precedence including
  the official-URL fallback, and an offline foreign-install end-to-end through
