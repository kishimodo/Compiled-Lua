@echo off
rem LuaVM differential fuzz campaign: generates seeded deterministic Lua
rem programs and diffs JIT stdout against the bytecode interpreter (-i).
rem
...
rem The products must already be built (build\build.bat luavm) -- the fixed
...
if not exist build\bin\luavm.exe (
    echo [-] build\bin\luavm.exe not found -- run build\build.bat luavm first
    popd & exit /b 1
)
