...
-- root by clua-interp.exe (so the compiler finds its build/bin libs).
...
local PROJ  = (os.getenv("TEMP") or ".") .. "\\clua-interp-mvtest"
...
local function pk(args)  -- pkg manager, run IN the project dir (writes toml/lock there)
  local ok, _, c = os.execute('cd /d "' .. PROJ .. '" && "' .. CLUA .. '" -i "' .. PKG .. '" ' .. args)
...
local function fail(m) print("[-] FAIL test-pkgmgr-multiversion: " .. m); sh(CLUA .. ' -i ' .. PKG .. ' remove vpkg >nul 2>&1'); os.exit(1) end
...
local iout = capture('cd /d "' .. PROJ .. '" && "' .. CLUA .. '" -i main.lua')
