...
local PROJ  = (os.getenv("TEMP") or ".") .. "\\clua-interp-depstest"
...
local function pkout(a) local p = io.popen('cd /d "' .. PROJ .. '" && "' .. CLUA .. '" -i "' .. PKG .. '" ' .. a .. ' 2>&1'); if not p then return "" end local o = p:read("*a") or ""; p:close(); return o end
...
local function cleanup() for _, n in ipairs({ "app", "mid", "leaf", "conflictdep" }) do sh(CLUA .. ' -i ' .. PKG .. ' remove ' .. n .. ' >nul 2>&1') end end
...
do
  ...
  local p = io.popen('cd /d "' .. PROJ .. '" && "' .. CLUA .. '" -i main.lua 2>&1')
