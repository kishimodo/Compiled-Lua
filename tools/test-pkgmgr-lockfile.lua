-- Lockfile-reproducibility test (Bug 2): a committed rover.lock must give
...
do
  local STORE = os.getenv("CLUA_HOME")
  if STORE and STORE ~= "" then STORE = STORE .. "\\packages"
  else STORE = (os.getenv("LOCALAPPDATA") or ".") .. "\\clua\\packages" end
