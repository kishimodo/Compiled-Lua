...
local function pk_env(env, a)
  ...
  local ok, _, c = os.execute('cd /d "' .. PROJ .. '" && ' .. pre .. '"' .. LUAVM .. '" -i "' .. PKG .. '" ' .. a)
...
local function fail(m) print("[-] FAIL test-pkgmgr-trust: " .. m); sh(LUAVM .. ' -i ' .. PKG .. ' remove leaf >nul 2>&1'); os.remove(SIGP); os.exit(1) end
...
if not sh('"' .. LUAVM .. '" -i "' .. PKG .. '" publish "' .. REGD .. '" >nul 2>&1') then fail("publish index") end
...
sh(LUAVM .. ' -i ' .. PKG .. ' remove leaf >nul 2>&1')
os.remove(SIGP)
sh('"' .. LUAVM .. '" -i "' .. PKG .. '" publish "' .. REGD .. '" >nul 2>&1')
