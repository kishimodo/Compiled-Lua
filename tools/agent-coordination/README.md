...
| Roadmap and status | [`docs/roadmaps/`](../../docs/roadmaps/) | one live status row per deliverable; edit this instead of re-litigating an audit. `language-platform.md` holds the tooling/language/reorg design and its execution order; `no-crt.md` is its companion track for CRT-free output |
