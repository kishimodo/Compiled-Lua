...
local function fail(m) print("[-] FAIL test-pkgmgr: " .. m); sh(LUAVM .. ' -i ' .. PKG .. ' remove greet >nul 2>&1'); os.exit(1) end

-- clean any prior copy, then install
sh(LUAVM .. ' -i ' .. PKG .. ' remove greet >nul 2>&1')
if not sh(LUAVM .. ' -i ' .. PKG .. ' install greet >nul 2>&1') then fail("install greet") end

-- idempotency: a forgotten reinstall must succeed, not corrupt
if not sh(LUAVM .. ' -i ' .. PKG .. ' install greet >nul 2>&1') then fail("reinstall (idempotency)") end
...
local iout = capture(LUAVM .. ' -i ' .. USES)
